...
# --- Configuration ---
file_path = "/Users/adityagirish/capstoned/synthetic_phugoid_flight.csv"
...
#     'hplwas', 'hplfd', 'vplwas', 'fqtyl', 'fqtyr', 'volt1', 'volt2', 'amp1', 'amp2','latitude','longitude'
# ]
...
df1 = pd.read_csv("/Users/adityagirish/capstoned/synthetic_phugoid_flight.csv")
...
# Identify columns with more than one unique value to avoid errors with PowerTransformer
columns_to_scale = [col for col in scalable_numeric_columns if col in df1.columns and df1[col].nunique() > 1]
skipped_columns = list(set(scalable_numeric_columns) - set(columns_to_scale))

if skipped_columns:
    print(f"Skipping scaling for low-variance or missing columns: {skipped_columns}")

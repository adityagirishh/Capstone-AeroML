...
# ✅ Normalize ALL features together (including lat/long/atmsl)
...
def create_windows(data, window_size=5, step=1):
    windows = []
...
# Instantiate model — increased embed_dim for better representation
input_dim = len(key_features)
model = LSTMAutoencoder(input_dim=input_dim, hidden_dim=64, embed_dim=32, dropout_rate=0.2)
...
model.eval()
with torch.no_grad():
    embeddings = model.encode(X_tensor).numpy()  # Shape: (n_windows, 32)

# =======================
# Step 3: HMM Modeling (Used for Final Labeling — More Reliable)
# =======================

hmm_model = hmm.GaussianHMM(n_components=5, covariance_type='diag', n_iter=100, random_state=42)
hmm_model.fit(data_scaled)
predicted_states = hmm_model.predict(data_scaled)  # Shape: (50,) — one per timestep!

print("\nPredicted HMM States for Each Timestep:")
